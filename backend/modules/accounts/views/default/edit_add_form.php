    ...
    <div class="card-footer text-right">
        <? if($model->scenario=='editProfile'){ ?>
            ...
            <a href="<?=\yii\helpers\Url::to(['/auth/auth','authclient'=>'google'])?>" id="google_button" class="btn btn-primary btn-outline">
                <?=$this->render('@app/web/quantum/google_logo.svg')?>&nbsp;&nbsp;Attach account Google
            </a>

            <a href="<?=\yii\helpers\Url::to(['/auth/auth','authclient'=>'facebook'])?>" id="facebook_button" class="btn btn-secondary btn-outline">
                <?=$this->render('@app/web/quantum/facebook.svg')?>&nbsp;&nbsp;Attach account Facebook
            </a>
    ...
    <? if(!Yii::$app->request->isAjax){ ?>
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <?=$form->field($model,'status')->dropDownList(Identity::$statuses)->label('Active')?>
                    </div>
                    <div class="col-md-4">
                        <?=$form->field($model,'wrong_pass_entered')->textInput()->label('Wrong Password Entered')?>
                    </div>
                </div>
